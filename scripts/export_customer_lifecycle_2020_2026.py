"""Export yearly 신규/기존/중단 customer lifecycle status to Excel for 2020~2026.
...
- Each yearly sheet includes monthly sales amount columns from 01월매출액 to 12월매출액.
"""
...
OUTPUT_FILE = OUTPUT_DIR / "customer_lifecycle_2020_2026.xlsx"
...
yearly_sales AS (
    SELECT
        ...
        SUM(CASE WHEN MONTH(bill_date) = 1 THEN sales_amt ELSE 0 END) AS `01월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 2 THEN sales_amt ELSE 0 END) AS `02월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 3 THEN sales_amt ELSE 0 END) AS `03월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 4 THEN sales_amt ELSE 0 END) AS `04월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 5 THEN sales_amt ELSE 0 END) AS `05월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 6 THEN sales_amt ELSE 0 END) AS `06월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 7 THEN sales_amt ELSE 0 END) AS `07월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 8 THEN sales_amt ELSE 0 END) AS `08월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 9 THEN sales_amt ELSE 0 END) AS `09월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 10 THEN sales_amt ELSE 0 END) AS `10월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 11 THEN sales_amt ELSE 0 END) AS `11월매출액`,
        SUM(CASE WHEN MONTH(bill_date) = 12 THEN sales_amt ELSE 0 END) AS `12월매출액`,
...
classified AS (
    SELECT
        ...
        CASE
            ...
            ELSE NULL
        END AS `신규기존중단여부`,
        COALESCE(curr.`01월매출액`, 0) AS `01월매출액`,
        COALESCE(curr.`02월매출액`, 0) AS `02월매출액`,
        COALESCE(curr.`03월매출액`, 0) AS `03월매출액`,
        COALESCE(curr.`04월매출액`, 0) AS `04월매출액`,
        COALESCE(curr.`05월매출액`, 0) AS `05월매출액`,
        COALESCE(curr.`06월매출액`, 0) AS `06월매출액`,
        COALESCE(curr.`07월매출액`, 0) AS `07월매출액`,
        COALESCE(curr.`08월매출액`, 0) AS `08월매출액`,
        COALESCE(curr.`09월매출액`, 0) AS `09월매출액`,
        COALESCE(curr.`10월매출액`, 0) AS `10월매출액`,
        COALESCE(curr.`11월매출액`, 0) AS `11월매출액`,
        COALESCE(curr.`12월매출액`, 0) AS `12월매출액`
...
def main_export() -> None:
    ...
    month_columns = [f"{month:02d}월매출액" for month in range(1, 13)]
    expected_columns = [
        "연도",
        "개인형/FC",
        "ZC코드",
